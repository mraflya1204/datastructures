...
int main(int argc, char const *argv[])
{
    ...
    SinglyList myListOdd; slist_init(&myListOdd);
    SinglyList myListEven; slist_init(&myListEven);
    ...
    for(int i = 0; i < N; i++){
        slist_pushBack(&myList, arr[i]);
    }

    SListNode *temp = myList._head;
    while(temp!=NULL){
        if(temp->data % 2 == 0 || temp->data % -2 == 0){
            slist_pushBack(&myListEven, temp->data);
        }
        else{
            slist_pushFront(&myListOdd, temp->data);
        }
        temp = temp->next;
    }
    free(temp);
    ...
    printf("Angka Genap:\n");
    SListNode *tempEven = myListEven._head;
    while(tempEven!=NULL){
        if(tempEven->next == NULL){
            printf("%d", tempEven->data);
        }
        else{
            printf("%d ", tempEven->data);
        }
    ...
    printf("Angka Ganjil:\n");
    SListNode *tempOdd = myListOdd._head;
    while(tempOdd!=NULL){
