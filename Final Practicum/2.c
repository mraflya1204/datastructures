...
void slist_sort_asc(SinglyList *list){
    int flag = -1;
    SListNode *headptr;
...
void slist_sort_desc(SinglyList *list){
    int flag = -1;
    SListNode *headptr;
...
int main(int argc, char const *argv[])
{
    SinglyList myList; 
    slist_init(&myList);
    SinglyList myListEven; 
    slist_init(&myListEven);
    SinglyList myListOdd; 
    slist_init(&myListOdd);
    ...
    slist_sort_asc(&myList);
    
    int countFirstList = 0;
    while(myList._head != NULL){
        if(countFirstList % 2 != 0){
            slist_pushBack(&myListEven, slist_front(&myList));
            slist_popFront(&myList);
        }
        else{
            slist_pushBack(&myListEven, slist_back(&myList));
            slist_popBack(&myList);
        }
        countFirstList++;
    }

    SListNode *temp = myListEven._head;
    while (temp != NULL) {
        printf("%d ", temp->data);
        temp = temp->next;
    }
